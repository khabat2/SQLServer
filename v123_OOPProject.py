...
def register(User):
    if User["Name"] == "":
        messagebox.showerror("فرم کامل نیست", "لطفا فیلد نام را وارد نمایید")
        return False
    elif User["Family"] == "":
        messagebox.showerror("فرم کامل نیست", "لطفا فیلد نام خانوادگی را وارد نمایید")
        return False
    elif User["Age"] == "":
        messagebox.showerror("فرم کامل نیست", "لطفا فیلد سن را وارد نمایید")
        return False
    else:
        # if int(User["Age"]) >= 18:
        tblName = "personal"
        ...
        #   val = "'" + Name.get()   + "','" + Family.get()   + "','" + Age.get()   + "' "
        result = repository.Create(tblName, col, val)
        if result:
...
def InsertData(val):
    tbl.insert('', "end", values=[val[2].get(), val[1].get(), val[0].get()])


#   tbl.insert(       '',      "end", values=[Code.get(),     Name.get(), Family.get(), Age.get()])
