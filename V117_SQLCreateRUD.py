...
def register(User):
    if int(User["Age"]) >= 18:
        if Exist(User):
            messagebox.showerror("هشدار", "فردی با این مشخصات قبلا ثبت گردیده")
            return False
        else:
            UserAll.append(User)
...
def Delete():
    ...
    if select_row != ():
        ...
        dic = {'Name': selectItem[2], 'Family': selectItem[1], 'Age': str(selectItem[0])}
...
def OnClickEdit():
    ...
    col = "prsl_name= '" + Name.get() + "', prsl_family= '" + Family.get() + "', prsl+age= '" + Age.get() + "' "
    ...
    if result:
        ...
        Clear(ListItem)
        messagebox.showinfo("ویرایش گردید", "ویرایش  با موفقیت انجام شد")
    else:
        messagebox.showerror(" خطا", "ویرایش با خطا مواجه شد")
...
btnDelete = Button(screen, text="حذف", bg="red", fg="white", width=6, command=OnClickDelete).place(x=370, y=110)
